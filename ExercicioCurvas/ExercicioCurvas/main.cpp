...
void GerenciaTecladoEspecial(int key, int x, int y)
{
	...
	case GLUT_KEY_PAGE_UP:
		if (bezier) {
			if (p1) BezierPoints[0][2] += 0.2;
			if (p2) BezierPoints[1][2] += 0.2;
			if (p3) BezierPoints[2][2] += 0.2;
			if (p4) BezierPoints[3][2] += 0.2;
		}
		if (hermite) {
			if (p1) HermitePoints[0][2] += 0.2;
			if (p2) HermitePoints[1][2] += 0.2;
			if (p3) HermitePoints[2][2] += 0.2;
			if (p4) HermitePoints[3][2] += 0.2;
		}
		if (bspline) {
			if (p1) BSplinePoints[0][2] += 0.2;
			if (p2) BSplinePoints[1][2] += 0.2;
			if (p3) BSplinePoints[2][2] += 0.2;
			if (p4) BSplinePoints[3][2] += 0.2;
		}
		break;
